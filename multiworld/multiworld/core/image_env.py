...
class ImageEnv(ProxyEnv, MultitaskEnv):
    def __init__(
        ...
        self.wrapped_env.unwrapped.hide_goal_markers = True
        self.imsize = imsize
        ...
        if init_camera is not None:
            self._wrapped_env.unwrapped.initialize_camera(init_camera)
        ...
        spaces = self.wrapped_env.unwrapped.observation_space.spaces.copy()
        ...
        self.action_space = self.wrapped_env.unwrapped.action_space
    ...
    def step(self, action):
        obs, reward, done, info = self.wrapped_env.unwrapped.step(action)
    ...
    def reset(self):
        obs = self.wrapped_env.unwrapped.reset()
        if self.num_goals_presampled > 0:
            goal = self.sample_goal()
            self._img_goal = goal['image_desired_goal']
            self.wrapped_env.unwrapped.set_goal(goal)
            for key in goal:
                obs[key] = goal[key]
            ...
            env_state = self.wrapped_env.unwrapped.get_env_state()
            self.wrapped_env.unwrapped.set_to_goal(self.wrapped_env.unwrapped.get_goal())
            self._img_goal = self._get_flat_img()
            self.wrapped_env.unwrapped.set_env_state(env_state)

        return self._update_obs(obs)

    def _get_obs(self):
        return self._update_obs(self.wrapped_env.unwrapped._get_obs())
    ...
    def _get_flat_img(self):
        image_obs = self._wrapped_env.unwrapped.get_image(
            width=self.imsize,
            height=self.imsize,
        )
    ...
    def render(self, mode='wrapped'):
        if mode == 'wrapped':
            self.wrapped_env.unwrapped.render()
        elif mode == 'cv2':
    ...
    """
    def get_goal(self):
        goal = self.wrapped_env.unwrapped.get_goal()
    ...
    def sample_goals(self, batch_size):
        ...
        goals = self.wrapped_env.unwrapped.sample_goals(batch_size)
        pre_state = self.wrapped_env.unwrapped.get_env_state()
        for i in range(batch_size):
            goal = self.unbatchify_dict(goals, i)
            self.wrapped_env.unwrapped.set_to_goal(goal)
            img_goals[i, :] = self._get_flat_img()
        self.wrapped_env.unwrapped.set_env_state(pre_state)
    ...
    def compute_rewards(self, actions, obs):
        if self.reward_type=='wrapped_env':
            return self.wrapped_env.unwrapped.compute_rewards(actions, obs)
    ...
    def get_diagnostics(self, paths, **kwargs):
        statistics = self.wrapped_env.unwrapped.get_diagnostics(paths, **kwargs)
