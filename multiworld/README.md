...
## Basic Usage
This library contains a variety of gymnasium `GoalEnv`s.

As a running example, let's say we have a `CarEnv`.
Like normal gymnasium envs, we can do
```
env = CarEnv()
obs = env.reset()
...
loaded via the `gymnasium.make` interface. To do so, they must first be registered:
...
env = gymnasium.make('SawyerPushNIPSEasy-v0')
```
...
The `compute_reward` interface is slightly different from gymnasium's.
