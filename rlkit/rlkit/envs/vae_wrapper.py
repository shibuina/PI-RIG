...
import numpy as np
try:
    # Try gymnasium first (newer)
    from gymnasium.spaces import Box, Dict
except ImportError:
    # Fall back to gym (older)
...
class VAEWrappedEnv(ProxyEnv, MultitaskEnv):
    ...
    def __init__(
        ...
        # Handle compatibility between gym and gymnasium
        # Ensure we use the same Dict class as the environment's spaces
        try:
            if hasattr(self.wrapped_env.observation_space, 'spaces'):
                # Get the Dict class from the same module as the environment spaces
                first_space = next(iter(self.wrapped_env.observation_space.spaces.values()))
                if hasattr(first_space, '__module__'):
                    if 'gymnasium' in first_space.__module__:
                        from gymnasium.spaces import Dict as GymnasiumDict
                        self.observation_space = GymnasiumDict(spaces)
                    else:
                        from gym.spaces import Box as GymBox, Dict as GymDict
                        # Convert gymnasium spaces to gym spaces if needed
                        gym_spaces = {}
                        for key, space in spaces.items():
                            if hasattr(space, 'low'):  # Box space
                                gym_spaces[key] = GymBox(
                                    low=space.low, high=space.high, 
                                    shape=space.shape, dtype=space.dtype
                                )
                            else:
                                gym_spaces[key] = space
                        self.observation_space = GymDict(gym_spaces)
                else:
                    self.observation_space = Dict(spaces)
            else:
                self.observation_space = Dict(spaces)
        except (AssertionError, TypeError) as e:
            # If Dict creation fails, try with gymnasium Dict
            try:
                from gymnasium.spaces import Dict as GymnasiumDict
                self.observation_space = GymnasiumDict(spaces)
            except ImportError:
                # If gymnasium is not available, try to convert spaces
                # Convert all spaces to gym.spaces format
                from gym.spaces import Box as GymBox, Dict as GymDict
                converted_spaces = {}
                for key, space in spaces.items():
                    if hasattr(space, 'low') and hasattr(space, 'high'):
                        # Convert to gym Box if it's box-like
                        converted_spaces[key] = GymBox(
                            low=np.array(space.low), 
                            high=np.array(space.high), 
                            dtype=space.dtype if hasattr(space, 'dtype') else np.float32
                        )
                    else:
                        converted_spaces[key] = space
                self.observation_space = GymDict(converted_spaces)
    ...
    Other functions
    """
    def add_mode(self, mode_name, mode_value):
        """Add a mode to the environment's mode map."""
        self._mode_map[mode_name] = mode_value
