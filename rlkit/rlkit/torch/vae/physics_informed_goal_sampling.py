...
class ReachabilityEstimator:
    ...
    def estimate_pusher_reachability(self, goal_state: torch.Tensor, current_state: torch.Tensor) -> torch.Tensor:
        ...
        max_hand_reach = 0.8  # Maximum hand movement
        max_puck_push = 0.5   # Maximum puck push distance
        
        hand_reachability = torch.exp(-hand_distance / max_hand_reach)
        puck_reachability = torch.exp(-puck_distance / max_puck_push)
    ...
    def estimate_pick_and_place_reachability(self, goal_state: torch.Tensor, current_state: torch.Tensor) -> torch.Tensor:
        ...
        # Hand reachability
        hand_distance = torch.norm(goal_hand - current_hand, dim=1)
        hand_reachability = torch.exp(-hand_distance / 0.6)  # Max arm reach
        
        # Object reachability (depends on whether it's grasped)
        obj_distance = torch.norm(goal_obj - current_obj, dim=1)
        obj_reachability = torch.exp(-obj_distance / 0.4)
    ...
    def estimate_reacher_reachability(self, goal_state: torch.Tensor, current_state: torch.Tensor) -> torch.Tensor:
        ...
        # Reachability based on joint and velocity changes
        joint_reachability = torch.exp(-joint_distance / np.pi)  # Joint changes
        velocity_reachability = torch.exp(-velocity_distance / 3.0)  # Velocity changes
...
class PhysicsInformedGoalSampler:
    ...
    def __init__(self, vae_model, physics_type='pusher', z_I_dim=4):
    ...
    def extract_physics_state(self, z_I: torch.Tensor) -> torch.Tensor:
        ...
        """
        if self.physics_type == 'pusher':
            # z_I contains [hand_x, hand_y, puck_x, puck_y]
            return z_I[:, :4]  # Return first 4 dimensions
        elif self.physics_type == 'pick_and_place':
            # z_I contains [gripper_opening, hand_xyz, obj_xyz]
            return z_I[:, :7]  # Return first 7 dimensions
        elif self.physics_type == 'reacher':
            # z_I contains [θ₁, θ₂, θ̇₁, θ̇₂]
            return z_I[:, :4]  # Return first 4 dimensions
        else:
            return z_I
...
def create_physics_informed_goal_sampler(vae_model, physics_type='pusher', z_I_dim=4):
    """Factory function to create a physics-informed goal sampler."""
    return PhysicsInformedGoalSampler(vae_model, physics_type, z_I_dim)
