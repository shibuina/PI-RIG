    ...
    if dataset_path is not None:
        ...
        if use_cached and osp.isfile(filename):
            ...
            if not isinstance(env, ImageEnv):
                ...
                imsize = env.wrapped_env.unwrapped.imsize
                env.wrapped_env.unwrapped.non_presampled_goal_img_is_garbage = non_presampled_goal_img_is_garbage
            env.wrapped_env.unwrapped.reset()
            info['env'] = env
                ...
                if random_and_oracle_policy_data:
                    ...
                    if i < num_random_steps:
                        ...
                        for _ in range(n_random_steps):
                            obs = env.unwrapped.step(env.unwrapped.action_space.sample())[0]
                    else:
                        ...
                        for _ in range(n_random_steps):
                            ...
                            obs, _, _, _ = env.unwrapped.step(action)
                elif oracle_dataset_using_set_to_goal:
                    goal = env.unwrapped.sample_goal()
                    env.wrapped_env.unwrapped.set_to_goal(goal)
                    obs = env.unwrapped._get_obs()
                elif random_rollout_data:
                    if i % n_random_steps == 0:
                        g = env.unwrapped.sample_goal()
                        env.wrapped_env.unwrapped.set_to_goal(g)
                    obs = env.unwrapped.step(env.unwrapped.action_space.sample())[0]
                else:
                    env.unwrapped.reset()
                    for _ in range(n_random_steps):
...
def grill_her_td3_experiment(variant):
    ...
    es = get_exploration_strategy(variant, env.unwrapped)
...
def get_video_save_func(rollout_function, env, policy, variant):
    ...
    if do_state_exp:
        ...
        image_env = env
        dump_video_kwargs['imsize'] = env.unwrapped.imsize
...
def temporary_mode(env, mode, func, args=None, kwargs=None):
    ...
    if kwargs is None:
        kwargs = {}
    cur_mode = env.unwrapped.cur_mode
    env.unwrapped.mode(env.unwrapped._mode_map[mode])
    return_val = func(*args, **kwargs)
    env.unwrapped.mode(cur_mode)
    return return_val
